...
export function useAgentSystem() {
  const [prefs, setPrefs] = useState<AgentPrefs>({ mode: "manual", threshold: 70, dailyCap: 10 });
  ...
  const updatePrefs = useCallback(async (updates: Partial<AgentPrefs>) => {
  ...
  const runAgents = useCallback(async (agents: string[]) => {
    setRunning(true);
    try {
