...
async function runBenchmarks() {
  console.log("\n╔══════════════════════════════════════════════════════════");
  console.log(
    `║ iCareerOS Daily Benchmark — ${new Date().toISOString().slice(0, 10)}`,
  );
  console.log("╠══════════════════════════════════════════════════════════");
  ...
  console.log(
    `║   High quality:  ${highQualityJobs?.toLocaleString() ?? "n/a"} (score ≥ 70)`,
  );
  ...
  const totalInserted = successful.reduce(
    (acc, r) => acc + (r.jobs_inserted ?? 0),
    0,
  );
  const boardsSeen = [...new Set(runs.map((r) => r.source_board))].filter(
    Boolean,
  );
  ...
  console.log(
    `║   Runs:          ${runs.length} (${successful.length} ok, ${failed.length} failed)`,
  );
  ...
  console.log(
    `║   Total:         ${discoveredTotal?.toLocaleString() ?? "n/a"}`,
  );
  console.log(
    `║   Last 24h:      ${discoveredRecent?.toLocaleString() ?? "n/a"}`,
  );
  ...
  console.log(
    `║   Alert prefs:   ${usersWithPrefs?.toLocaleString() ?? "n/a"} (alerts enabled)`,
  );
  ...
  if (flags) {
    ...
    const disabledFlags = typedFlags
      .filter((f) => !f.enabled)
      .map((f) => f.key);
    console.log(`║`);
    ...
    if (disabledFlags.length)
      console.log(`║   Disabled: ${disabledFlags.join(", ")}`);
  }
  ...
  if (!healthOk) {
    console.warn(
      "⚠️  No new jobs ingested in last 24 hours and scraper is running",
    );
  }
