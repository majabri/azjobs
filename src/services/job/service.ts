...
export function normalizeJobUrl(rawUrl?: string | null): string {
  ...
  const trackingParams =
    /([&?](utm_[^=]*|gclid|fbclid|msclkid|mc_cid|mc_eid|_hsenc|_hsmi|igshid|si|spm)=[^&]*)/g;
...
export async function searchJobs(filters: JobSearchFilters): Promise<{
  jobs: JobResult[];
  citations: string[];
  ...
  try {
    ...
    const { data, error } =
      await supabase.functions.invoke<DiscoverJobsResponse>("discover-jobs", {
        body: payload,
      });

    if (error || !data) {
      logger.warn(
        "[searchJobs] Edge function failed, falling back to DB:",
        error?.message,
      );
...
export async function pollMatchScores(
  jobIds: string[],
): Promise<Map<string, Partial<JobResult>>> {
  ...
  const { data, error } = await (supabase as any)
    ...
    .select(
      "job_id, fit_score, matched_skills, skill_gaps, strengths, red_flags, match_summary, effort_level, response_prob, smart_tag",
    )
...
function buildJobTypeFilter(jobTypes: string[]): string | undefined {
  const structural = jobTypes.filter(
    (t) => !["remote", "hybrid", "in-office"].includes(t),
  );
...
export async function searchDatabaseJobsFallback(
  ...
  try {
    ...
    const POSTGREST_RESERVED = new Set([
      "and",
      "or",
      "not",
      "in",
    ...
    const COLS =
      "id, title, company, location, job_type, description, job_url, quality_score, is_flagged, flag_reasons, salary, market_rate, seniority, is_remote, source, first_seen_at";
    ...
    const buildFilteredQuery = (orParts: string[]) => {
      let q = (supabase as any).from("scraped_jobs").select(COLS);
      ...
      const salaryMin = filters.salaryMin
        ? parseInt(filters.salaryMin.replace(/\D/g, ""), 10)
        : null;
      const salaryMax = filters.salaryMax
        ? parseInt(filters.salaryMax.replace(/\D/g, ""), 10)
        : null;
    ...
    const toResult = (
      ...
      seniority: (row.seniority as string | null) ?? undefined,
      is_remote: (row.is_remote as boolean | null) ?? undefined,
      source: (row.source as string) || "database",
      first_seen_at: (row.first_seen_at as string | null) ?? undefined,
    });
    ...
    if (filters.query) {
      ...
      qKws
        .slice(0, 4)
        .forEach((kw) => titleOrParts.push(`title.ilike.%${kw}%`));
    }
    ...
    if (titleOrParts.length > 0) {
      ...
      if (error)
        logger.error(
          "[searchDatabaseJobsFallback] title pass error:",
          error.message,
        );
      titleJobs = (data as any[]) || [];
    }
    ...
    const descOrParts: string[] = skillPhrases
      .slice(0, 8)
    ...
    if (filters.query) {
      ...
      qKws
        .slice(0, 4)
        .forEach((kw) => descOrParts.push(`description.ilike.%${kw}%`));
    }
    ...
    if (descOrParts.length > 0) {
      ...
      if (error)
        logger.error(
          "[searchDatabaseJobsFallback] skill pass error:",
          error.message,
        );
      // Exclude jobs already returned by the title pass
      skillJobs = ((data as any[]) || []).filter(
        (j) => !titleJobIds.has(j.id as string),
      );
    }
...
export async function searchAIJobs(): Promise<{
  jobs: JobResult[];
  citations: string[];
}> {
