...
async function main() {
  ...
  const { data: rawJobs, error } = await supabase
    .from("raw_jobs")
    .select(
      "id, source, source_job_id, title, company, location, remote_type, salary_min, salary_max, raw_html, raw_json",
    )
  ...
  console.log(
    `[Extractor] ${unextracted.length} unextracted jobs found (from ${rawJobs?.length ?? 0} total)`,
  );
  ...
  for (let i = 0; i < unextracted.length; i += batchSize) {
    ...
    const results = await Promise.allSettled(
      batch.map((job) => extractAndSave(job)),
    );
    ...
    console.log(
      `[Extractor] Progress: ${processed + failed}/${unextracted.length} | ${rate} jobs/sec | ${failed} failed`,
    );
  }
  ...
  console.log(
    `  Cost est.: ~$${(processed * 0.00025).toFixed(2)} (Claude Haiku @ $0.25/1M tokens)`,
  );
  ...
  if (errorRate > 0.2) {
    console.error(
      `❌ Error rate ${Math.round(errorRate * 100)}% exceeds 20% threshold`,
    );
    process.exit(1);
  }
}
...
async function extractAndSave(job: any): Promise<void> {
  ...
  if ((!extracted || extracted.confidence_score < 0.7) && ANTHROPIC_KEY) {
...
async function extractWithOllama(
  ...
  return parseJsonResponse(data.response, "mistral");
}
...
function buildJobText(job: any): string {
  ...
  return [job.title, job.company, job.location, j.description ?? ""]
    .filter(Boolean)
    .join("\n")
    .slice(0, 5000);
}
...
function parseJsonResponse(text: string, method: string): any {
  ...
  return {
    ...
    remote_type: VALID_REMOTE.includes(parsed.remote_type)
      ? parsed.remote_type
      : "unknown",
    required_skills: Array.isArray(parsed.required_skills)
      ? parsed.required_skills.map(String).slice(0, 20)
      : [],
    experience_level: VALID_EXP.includes(parsed.experience_level)
      ? parsed.experience_level
      : "unknown",
    employment_type: VALID_EMP.includes(parsed.employment_type)
      ? parsed.employment_type
      : "unknown",
    job_description_clean: String(parsed.job_description_clean ?? "").slice(
      0,
      2000,
    ),
    salary_min:
      typeof parsed.salary_min === "number" ? parsed.salary_min : null,
    salary_max:
      typeof parsed.salary_max === "number" ? parsed.salary_max : null,
    currency: "USD",
    confidence_score: Math.min(
      1,
      Math.max(0, Number(parsed.confidence_score ?? 0.75)),
    ),
