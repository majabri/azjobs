...
describe("scoringService", () => {
  describe("buildSkillMatches", () => {
    it("should correctly identify matched and unmatched skills", () => {
      ...
      expect(matches.find(m => m.skill === "Kubernetes")?.matched).toBe(false);
      
      // Confidence checks
      expect(matches.find(m => m.skill === "React")?.confidence).toBeGreaterThanOrEqual(70);
  ...
  describe("buildGaps", () => {
    it("should generate structured gaps for unmatched skills", () => {
      const matches = [
        { skill: "AWS", matched: false, confidence: 0, context: "", category: "Cloud" },
        { skill: "Docker", matched: false, confidence: 0, context: "", category: "Cloud" },
        { skill: "React", matched: true, confidence: 80, context: "", category: "Frontend" },
        { skill: "Jest", matched: false, confidence: 0, context: "", category: "Testing" },
        { skill: "CSS", matched: false, confidence: 0, context: "", category: "Frontend" },
        { skill: "Figma", matched: false, confidence: 0, context: "", category: "Design" },
        { skill: "SQL", matched: false, confidence: 0, context: "", category: "Backend" },
      ];
  ...
  describe("computeExperienceMatch", () => {
    it("should penalize score for extreme job level mismatches", () => {
      ...
      const match1 = computeExperienceMatch("Chief Technology Officer", "Entry-level developer intern");
      expect(match1).toBeLessThan(50);
      
      // Senior applying to Senior
      const match2 = computeExperienceMatch("Senior Engineer", "Senior Developer");
  ...
  describe("buildTopActions", () => {
    it("should return the top 3 targeted actions to fix extreme gaps", () => {
      const gaps = buildGaps([{ skill: "Security Architecture", matched: false, confidence: 0, context: "", category: "Cybersecurity" }]);
