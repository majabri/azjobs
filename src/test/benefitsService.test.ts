...
describe("benefitsService", () => {
  describe("extractBenefits", () => {
    it("should correctly identify standard benefits from text blocks", () => {
      const fullJobText =
        "We offer a competitive salary, 401(k) matching, and generous paid time off.";
    ...
    it("should extract salary metadata if a valid range string is provided", () => {
      const fullJobText =
        "Base salary is $150k - $200k/year depending on experience.";
      const benefitsSectionText = "";
    ...
    it("should correctly bypass exclusion patterns disguised as benefits", () => {
      const fullJobText =
        "You must have 5+ years of experience with medical insurance billing software.";
      ...
      expect(
        benefits.find((b) => b.category === "health_insurance"),
      ).toBeUndefined();
    });
