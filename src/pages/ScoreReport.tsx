...
export default function ScoreReport() {
  ...
  useEffect(() => {
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps -- loadReport intentionally excluded; mount-only fetch
  }, [analysisId]);
  ...
  useEffect(() => {
    ...
    if (report) {
      ...
      const desc =
        report.summary ||
        `Scored ${report.overall_score}% on iCareerOS career analysis`;
  ...
  const loadReport = async () => {
    try {
      const { data, error } = await supabase
        ...
        .select(
          "overall_score, matched_skills, gaps, strengths, improvement_plan, job_title, company, summary, created_at",
        )
        .eq("id", analysisId!)
        .maybeSingle();
    ...
    if (navigator.share) {
      try {
        await navigator.share({
          ...
          text:
            report.summary || `I scored ${report.overall_score}% on iCareerOS`,
      ...
      <div className="min-h-screen bg-background flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-muted-foreground">
            This score report doesn't exist or is no longer available.
          </p>
        </div>
      </div>
    );
  ...
  const matchedSkills = (
    Array.isArray(report.matched_skills) ? report.matched_skills : []
  ) as Array<{ matched: boolean; [k: string]: unknown }>;
  const matched = matchedSkills.filter((s) =>
    typeof s === "object" ? s.matched : true,
  );
  const unmatched = matchedSkills.filter((s) =>
    typeof s === "object" ? !s.matched : false,
  );
  const gaps = (Array.isArray(report.gaps) ? report.gaps : []) as Array<
  ...
  const interviewProb = Math.min(
    95,
    Math.max(5, report.overall_score * 0.8 + strengths.length * 3),
  );
    ...
    <div className="min-h-screen bg-background">
      <div className="max-w-2xl mx-auto px-6 py-12 space-y-6">
        ...
        <div className="text-center">
          ...
          <div
            className={`w-28 h-28 rounded-full border-4 flex items-center justify-center mx-auto mb-4 ${ringBorder} ${ringBg}`}
          >
            <div className="text-center">
        ...
        <div className="grid grid-cols-3 gap-3">
          ...
          <Card className="p-3 text-center">
            ...
            <p className="font-display font-bold text-primary text-lg">
              {matched.length}/{matchedSkills.length}
            </p>
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-sm font-semibold text-foreground">
                  Fixing all gaps could increase your score by +{gapImpact}%
                </p>
                <p className="text-xs text-muted-foreground">
                  Estimated interview probability:{" "}
                  {Math.min(95, interviewProb + gapImpact)}%
                </p>
              </div>
            </div>
          </Card>
        ...
        {gaps.length > 0 && (
          <Card className="p-5">
            <h3 className="font-semibold text-primary mb-3 flex items-center gap-2">
              <XCircle className="w-4 h-4 text-destructive" /> Gaps to Address
            </h3>
                ...
                return (
                  ...
                  >
                    <span className="text-sm text-foreground">
                      {typeof g === "object" ? g.area : g}
                    </span>
                    <Badge
                      variant="outline"
                      className="text-accent border-accent/30 text-[10px]"
                    >
                      <ArrowUpRight className="w-3 h-3 mr-0.5" />+{delta}% if
                      fixed
                    </Badge>
                  </div>
                );
        ...
        {plan.length > 0 && (
          <Card className="p-5">
            <h3 className="font-semibold text-primary mb-3 flex items-center gap-2">
              <Zap className="w-4 h-4 text-accent" /> Improvement Plan
            </h3>
        ...
        <Card className="p-5 text-center border-accent/20 bg-accent/5">
          <p className="text-sm text-foreground mb-2">
            Want your own Career Report Card?
          </p>
          <Button
            size="sm"
            className="gradient-indigo text-white"
        ...
        <div className="text-center pt-6 border-t border-border">
          <p className="text-xs text-muted-foreground">
            Powered by iCareerOS — AI Career Intelligence
          </p>
        </div>
      </div>
    </div>
