...
} from "@/hooks/queries/useInterviewSchedules";
...
export default function InterviewScheduling() {
  ...
  const jobPostings: JobPostingOption[] = jobPostingsData.map((jp) => ({
    id: jp.id,
    title: jp.title,
    company: jp.company ?? "",
  }));
  ...
  const resetForm = () => {
    ...
    setFeedback("");
    setRating("");
    setJobPostingId("none");
    setEditing(null);
  };

  const openEdit = (i: InterviewScheduleRow) => {
    setEditing(i);
    setCandidateName(i.candidate_name);
    setCandidateEmail(i.candidate_email || "");
    setInterviewType(i.interview_type);
    setScheduledAt(i.scheduled_at.slice(0, 16));
    setDuration(String(i.duration_minutes));
    setLocationVal(i.location || "");
    setMeetingLink(i.meeting_link || "");
    setNotes(i.notes || "");
    setStatus(i.status);
    setFeedback(i.feedback || "");
    setRating(i.rating?.toString() || "");
    setJobPostingId(i.job_posting_id || "none");
    setDialogOpen(true);
  };
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            {upcoming.length} upcoming interviews
          </p>
        </div>
        <Dialog
          open={dialogOpen}
        ...
        >
          ...
          <DialogContent className="max-w-lg max-h-[85vh] overflow-y-auto">
            <DialogHeader>
              <DialogTitle>
                {editing ? "Edit" : "Schedule"} Interview
              </DialogTitle>
            </DialogHeader>
        ...
        <div className="flex items-start justify-between">
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2 mb-1">
              ...
              <Badge className={statusColors[i.status] || ""} variant="outline">
                {i.status}
              </Badge>
