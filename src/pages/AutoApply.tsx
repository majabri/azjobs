...
import { Bot, Play, Eye, Shield, Zap } from "lucide-react";
...
import {
  QueuedJobCard,
  QueuedApplication,
} from "@/components/auto-apply/QueuedJobCard";
...
export default function AutoApplyPage() {
  ...
  const addLog = (
    ...
    setActivityLog((prev) =>
      [{ timestamp: new Date(), action, detail, type }, ...prev].slice(0, 50),
    );
  };
  ...
  const loadProfileDefaults = async () => {
    try {
      ...
      const { data } = await supabase
        ...
        .select(
          "target_job_titles, location, preferred_job_types, salary_min, salary_max, remote_only, min_match_score",
        )
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(async () => {
      try {
        ...
        await supabase
          .from("job_seeker_profiles")
          .update({ min_match_score: prefs.minMatchScore })
          .eq("user_id", session.user.id);
      } catch (e) {
        logger.error("Failed to save match score:", e);
      }
    }, 500);
  ...
  const buildProfileResumeFallback = (profile: any): string => {
    ...
    const workExp = Array.isArray(profile.work_experience)
      ? profile.work_experience
          .map((w: any) =>
            `${w?.title || ""} at ${w?.company || ""}. ${w?.description || ""}`.trim(),
          )
          .filter(Boolean)
      : [];
    ...
    const certs = Array.isArray(profile.certifications)
      ...
      : "";
    const titles = Array.isArray(profile.target_job_titles)
  ...
  const getBestResumeText = async (
    userId: string,
  ): Promise<{
    text: string;
    source: "resume_version" | "profile" | "none";
  }> => {
    ...
    const { data: profile } = await supabase
      .from("job_seeker_profiles")
      .select(
        "full_name, summary, skills, work_experience, certifications, target_job_titles, career_level",
      )
      .eq("user_id", userId)
      .maybeSingle();
  ...
  const runAutoSearch = async () => {
    ...
    try {
      ...
      const { data, error: searchError } = await supabase.functions.invoke(
        "search-jobs",
        {
          body: {
            ...
            location:
              prefs.locations.join(", ") || (profile?.location as string) || "",
      ...
      const newQueue: QueuedApplication[] = jobs
        .map((job: any) => {
          ...
          const analysis = resumeText
            ? analyzeJobFit(jobDesc, resumeText)
            : null;
        ...
        .sort(
          (a: QueuedApplication, b: QueuedApplication) =>
      ...
      addLog(
        "Search complete",
        `Found ${newQueue.filter((j) => j.status === "review").length} matching jobs`,
        "search",
      );
      ...
      if (prefs.applyMode === "smart" || prefs.applyMode === "full-auto") {
        const threshold =
          prefs.applyMode === "full-auto" ? prefs.minMatchScore : 80;
        for (const job of newQueue) {
          if (job.matchScore >= threshold && job.status === "review") {
            addLog(
              "Auto-approved",
              `${job.jobTitle} at ${job.company} (${job.matchScore}%)`,
              "apply",
            );
          }
        }
      ...
      if (resumeLookup.source === "profile") {
        toast.success(
          `Found ${newQueue.filter((j) => j.status === "review").length} jobs. Using your profile for fit analysis.`,
        );
      } else {
        toast.success(
          `Found ${newQueue.filter((j) => j.status === "review").length} jobs above ${prefs.minMatchScore}% threshold`,
        );
      }
    } catch {
  ...
  const generatePackage = async (item: QueuedApplication) => {
    ...
    try {
      ...
      const resumeText = resumeLookup.text;
      if (!resumeText) {
        toast.error("No resume/profile content found.");
        return;
      }

      const analysis =
        item.analysis || analyzeJobFit(item.jobDescription || "", resumeText);
            ...
            for (const line of chunk.split("\n")) {
              if (line.startsWith("data: ") && line !== "data: [DONE]") {
                try {
                  ...
                  optimizedResume += p.choices?.[0]?.delta?.content || "";
                } catch (e) {
          ...
          while (true) {
            ...
            for (const line of chunk.split("\n")) {
              if (line.startsWith("data: ") && line !== "data: [DONE]") {
                try {
                  const p = JSON.parse(line.slice(6));
                  coverLetter += p.choices?.[0]?.delta?.content || "";
                } catch (e) {
                  logger.warn("SSE parse error:", e);
                }
              }
  ...
  const skipJob = (id: string) => {
    setQueue((prev) =>
      prev.map((q) => (q.id === id ? { ...q, status: "skipped" as const } : q)),
    );
  };
            ...
            [
              {
                ...
                label: "Manual Review",
                desc: "You review every job before applying",
                icon: Eye,
              },
              {
                mode: "smart" as const,
                label: "Smart Approval",
                desc: "Auto-approve jobs above 80% match",
                icon: Zap,
              },
              {
            ...
            <Card
              ...
              onClick={() =>
                setPrefs({
                  ...prefs,
                  applyMode: m.mode,
                  requireReview: m.mode === "manual",
                })
              }
            >
              <div className="flex items-center gap-2 mb-1">
                <m.icon
                  className={`w-4 h-4 ${prefs.applyMode === m.mode ? "text-accent" : "text-muted-foreground"}`}
                />
                <span
                  className={`text-sm font-semibold ${prefs.applyMode === m.mode ? "text-accent" : "text-foreground"}`}
                >
                  {m.label}
                </span>
              </div>
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-sm font-semibold text-foreground">
                {prefs.applyMode === "full-auto"
                  ? "Your AI is applying to jobs right now"
                  : prefs.applyMode === "smart"
                    ? "Smart Approval — auto-applying to 80%+ matches"
                    : "Manual Review Mode"}
              </p>
              <p className="text-xs text-muted-foreground">
                {prefs.applyMode === "full-auto"
                  ? "AI finds, optimizes, and queues applications automatically."
                  : prefs.applyMode === "smart"
                    ? "Jobs above 80% match are auto-approved. Others need your review."
        ...
        <Card className="p-5 border-dashed border-2 border-muted">
          ...
          <div className="grid sm:grid-cols-2 gap-4 text-sm text-muted-foreground">
            <div className="flex gap-2">
              ...
              <span>Searches jobs matching your preferences & skills</span>
            </div>
            <div className="flex gap-2">
              <span className="text-accent font-bold">2.</span>
              <span>You review job descriptions & analyze fit</span>
            </div>
            <div className="flex gap-2">
            ...
            <div className="flex gap-2">
              ...
              <span>Approve and track in your application dashboard</span>
            </div>
          </div>
          <p className="text-xs text-muted-foreground mt-3 flex items-center gap-1">
            <Shield className="w-3 h-3" /> Nothing happens without your review
            and approval.
          </p>
        </Card>
      </div>
