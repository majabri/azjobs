          ...
          while (true) {
            ...
            for (const line of chunk.split("\n")) {
              if (line.startsWith("data: ") && line !== "data: [DONE]") {
                try { const p = JSON.parse(line.slice(6)); optimizedResume += p.choices?.[0]?.delta?.content || ""; } catch (_) { /* ignore non-JSON SSE lines */ }
              }
            }
          }
        }
      ...
      if (coverResp.ok) {
        ...
        if (reader) {
          while (true) {
            ...
            for (const line of chunk.split("\n")) {
              if (line.startsWith("data: ") && line !== "data: [DONE]") {
                try { const p = JSON.parse(line.slice(6)); coverLetter += p.choices?.[0]?.delta?.content || ""; } catch (_) { /* ignore non-JSON SSE lines */ }
              }
            }
          }
        }
