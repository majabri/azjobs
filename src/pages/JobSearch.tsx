...
export default function JobSearchPage() {
  ...
  const [sortBy, setSortBy] = useState<
    "relevance" | "probability" | "newest" | "decision" | "fit_score"
  >("fit_score");
  ...
  const loadIgnoredAndSaved = async () => {
    ...
    ] = await Promise.all([getIgnoredJobs(), supabase.auth.getSession()]);
    ...
    try {
      ...
      const [{ data }, { data: appData }] = await Promise.all([
        supabase
          ...
          .select(
            "skills, preferred_job_types, location, career_level, target_job_titles, salary_min, salary_max, min_match_score, search_mode",
          )
          .eq("user_id", session.user.id)
          .maybeSingle(),
        supabase
      ...
      if (data) {
        ...
        if (data.preferred_job_types)
          setJobTypes(data.preferred_job_types as string[]);
        if (data.location && data.location !== "<UNKNOWN>")
          setLocation(data.location);
        if (data.career_level) setCareerLevel(data.career_level);
        if (data.target_job_titles)
          setTargetTitles(data.target_job_titles as string[]);
        ...
        if (data.search_mode)
          setSearchMode(data.search_mode as "quality" | "balanced" | "volume");
      }
      if (appData && appData.length >= 3) {
        const total = appData.length;
        const responded = appData.filter(
        ...
        const avgDays =
          days.length > 0 ? days.reduce((a, b) => a + b, 0) / days.length : 14;
        setHistoricalOutcomes({
      ...
      logger.error("[JobSearch] scrape error:", e);
    } finally {
      setProfileLoaded(true);
    }
  };
  ...
  const handleSearch = async (overrideFilters?: Partial<JobSearchFilters>) => {
    ...
    try {
      ...
      const msg =
        e instanceof Error ? e.message : "Search encountered an issue.";
      setSearchError(msg);
    ...
    enriched = enriched
      .filter((job) => !isJobIgnored(job as any, ignoredList))
      .filter((job) => !isJobAlreadySaved(job as any, savedApps));
    ...
    if (!enriched.length && beforeCount > 0) {
      toast.info(
        `${beforeCount} jobs found but hidden by your ${effectiveMinFit}% fit score filter.`,
      );
    ...
    if (triggeredMatch && enriched.length > 0) {
      schedulePollForScores(
        enriched.map((j) => j.id).filter(Boolean) as string[],
      );
    }
  };
    ...
    pollTimerRef.current = setTimeout(async () => {
      ...
      if (scoreMap.size > 0) {
        ...
        if (!allScored) {
          ...
          setMatchingInProgress(false);
          toast.success(
            "AI fit scores ready — jobs re-ranked by match quality.",
          );
        }
      } else {
  ...
  const sortResults = (list: EnrichedJob[], by: string): EnrichedJob[] => {
    ...
    if (by === "fit_score" || by === "decision") {
      ...
      sorted.sort(
        (a, b) =>
          (b.response_prob ?? b.responseProbability ?? 0) -
          (a.response_prob ?? a.responseProbability ?? 0),
      );
    } else if (by === "newest") {
      sorted.sort((a, b) => {
  ...
  const handleAnalyzeFit = (job: EnrichedJob) => {
    ...
    navigate("/job-seeker", {
      state: { prefillJob: jobDesc, fromSearch: true },
    });
  };
      ...
      <div className="max-w-5xl mx-auto px-6 py-10">
        ...
        <div className="text-center mb-10">
          <h1 className="font-display text-4xl font-bold text-primary mb-3">
            Jobs that{" "}
            <span className="text-gradient-indigo">get you interviews</span>
          </h1>
          <p className="text-muted-foreground text-lg max-w-xl mx-auto">
            AI scores every job against your profile — so you focus on the ones
            worth applying to.
          </p>
        </div>
        ...
        <Card className="p-6 mb-8">
          <div className="space-y-4">
              ...
              <div className="flex flex-wrap gap-2">
                {targetTitles.map((t, i) => (
                  <Badge
                    ...
                    onClick={() =>
                      setTargetTitles(
                        targetTitles.filter((_, idx) => idx !== i),
                      )
                    }
                  >
              ...
              <div className="flex flex-wrap gap-2">
                {[
                  ...
                  const levels = careerLevel
                    ? careerLevel.split(", ").filter(Boolean)
                    : [];
            ...
            <div className="grid sm:grid-cols-2 gap-4">
              ...
              <div>
                ...
                <Input
                  ...
                  placeholder="e.g. startup, Fortune 500..."
                />
              </div>
            </div>
            ...
            <div className="grid sm:grid-cols-2 gap-4">
              ...
              <div>
                ...
                <select
                  ...
                  onChange={(e) =>
                    setSearchMode(
                      e.target.value as "quality" | "balanced" | "volume",
                    )
                  }
                >
            ...
            <div>
              <label className="text-sm font-semibold text-foreground mb-2 block">
                Minimum Fit Score:{" "}
                <span className="text-accent font-bold">{minFitScore}%</span>
              </label>
              <p className="text-xs text-muted-foreground mb-2">
                Hide jobs below this AI fit score threshold
              </p>
            ...
            <Button
              className="gradient-indigo text-white shadow-indigo-500/20 hover:opacity-90 w-full sm:w-auto"
              disabled={searching}
          ...
          <div className="mb-4 flex items-center gap-3 rounded-lg border border-primary/30 bg-primary/5 px-4 py-3">
            ...
            <div className="flex-1">
              <p className="text-sm font-medium text-primary">
                Analyzing your fit score for each job…
              </p>
              <p className="text-xs text-muted-foreground">
                Results are re-ranking as AI scores come in.{" "}
                {aiScoredCount > 0 ? `${aiScoredCount} scored so far.` : ""}
              </p>
            </div>
            ...
            {skills.length > 0 || targetTitles.length > 0 ? (
              <>
                ...
                <p className="text-sm mb-4">
                  Click Search Jobs to find live matches for your profile.
                </p>
              </>
            ) : (
              <>
                <p className="text-lg mb-2">
                  Set up your profile to get personalized results
                </p>
                <p className="text-sm mb-4">
                  Add skills and target titles to your{" "}
                  <button
                    className="text-accent hover:underline"
                    onClick={() => navigate("/profile")}
                  >
        ...
        {!searching && !searchError && searchRan && jobs.length === 0 && (
          <div className="text-center py-16 text-muted-foreground">
            ...
            <p className="text-sm mb-4">
              Try broadening your criteria — lower the fit score, remove the
              location filter, or add more job titles.
            </p>
            <Button
              variant="outline"
              onClick={() =>
