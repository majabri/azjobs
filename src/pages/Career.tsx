...
export default function CareerPage() {
  ...
  const analyzeCareer = async () => {
    ...
    try {
      ...
      const { data: insightData, error: insightError } =
        await supabase.functions.invoke("career-path-analysis", {
          body: {
  ...
  const targetTitles = profile?.target_job_titles || [];
  const salaryMin = profile?.salary_min || "";
  const salaryMax = profile?.salary_max || "";
  const profileSalaryTarget = profile?.salary_target || "";
  const profileGoalsShort = profile?.career_goals_short || "";
  const profileGoalsLong = profile?.career_goals_long || "";
    ...
    <div className="bg-background">
      <div className="max-w-6xl mx-auto px-6 py-8 space-y-8">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
          ...
          <Card className="p-4">
            ...
            <p className="font-display font-bold text-accent">
              {profileSalaryTarget ? `$${profileSalaryTarget}` : "Not set"}
            </p>
          </Card>
        </div>
        ...
        <Card className="p-6">
          <div className="flex items-center justify-between mb-4">
            <h2 className="font-display font-bold text-primary text-lg flex items-center gap-2">
              <Target className="w-5 h-5 text-accent" /> Career Goals
            </h2>
            <Button
              variant="ghost"
              size="sm"
              onClick={() => (editingGoals ? saveGoals() : handleEditClick())}
          ...
          {editingGoals ? (
            <div className="space-y-4">
              <div>
                ...
                <Textarea
                  value={goalsShort}
                  onChange={(e) => setGoalsShort(e.target.value)}
                  placeholder="e.g., Land a senior engineer role at a Series B+ startup"
                  className="mt-1"
                />
              </div>
              <div>
                ...
                <Textarea
                  ...
                  placeholder="e.g., Transition into engineering management or VP of Engineering"
                  className="mt-1"
                />
              </div>
              <div>
            ...
            <div className="space-y-3">
              <div>
                ...
                <p className="text-sm text-foreground">
                  {profileGoalsShort || "No short-term goal set yet"}
                </p>
              </div>
              <div>
                <p className="text-xs text-muted-foreground">Long-term</p>
                <p className="text-sm text-foreground">
                  {profileGoalsLong || "No long-term goal set yet"}
                </p>
              </div>
            </div>
          )}
        ...
        {scoreTrend.length > 1 && (
          <Card className="p-6">
            <h2 className="font-display font-bold text-primary text-lg flex items-center gap-2 mb-4">
              <BarChart3 className="w-5 h-5 text-accent" /> Score Improvement
              Trend
            </h2>
            <div className="h-48">
        ...
        <Card className="p-6">
          <div className="flex items-center justify-between mb-4">
            <h2 className="font-display font-bold text-primary text-lg flex items-center gap-2">
              <TrendingUp className="w-5 h-5 text-accent" /> AI Career Roadmap
            </h2>
            <Button
              variant="outline"
              size="sm"
            ...
            >
              {analyzing ? (
                ...
                <>
                  <Sparkles className="w-4 h-4 mr-1" /> Generate Roadmap
                </>
              )}
            </Button>
          </div>
          ...
          {!insight ? (
            <div className="text-center py-8">
              ...
              <p className="text-muted-foreground">
                Generate your personalized career roadmap based on your profile,
            ...
            <div className="space-y-6">
                ...
                <div className="relative">
                  ...
                  {insight.roadmap.map((stage, i) => (
                    <div key={i} className="relative pl-12 pb-6 last:pb-0">
                      <div
                        className={`absolute left-3.5 w-3 h-3 rounded-full border-2 ${i === 0 ? "bg-accent border-accent" : "bg-card border-border"}`}
                      />
              ...
              <div>
                <h4 className="text-sm font-semibold text-primary mb-3 flex items-center gap-1">
                  <GraduationCap className="w-4 h-4" /> Skills to Unlock Next
                  Level
                </h4>
              ...
              {insight.industryTrends?.length > 0 && (
                <div>
                  ...
                  <ul className="space-y-1">
                    {insight.industryTrends.map((t, i) => (
                      <li
                        key={i}
                        className="text-sm text-muted-foreground flex items-start gap-2"
                      >
                        <Sparkles className="w-3 h-3 mt-1 text-accent flex-shrink-0" />
                        {t}
                      </li>
                    ))}
                  </ul>
        ...
        <div className="space-y-6">
          <h2 className="font-display font-bold text-primary text-xl flex items-center gap-2">
            <Users className="w-5 h-5 text-accent" /> Networking & Outreach
          </h2>
          <OutreachGenerator />
          <OutreachTracker />
        </div>
