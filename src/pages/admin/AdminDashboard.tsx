...
export default function AdminDashboard() {
  ...
  const load = async () => {
    setLoading(true);
    try {
      const [profilesRes, analysesRes, applicationsRes, agentRunsRes] =
        await Promise.all([
          supabase
            .from("job_seeker_profiles")
            .select("user_id, last_active_at", { count: "exact" }),
          supabase.from("analysis_history").select("id", { count: "exact" }),
          supabase.from("job_applications").select("id", { count: "exact" }),
          supabase
            .from("agent_runs")
            .select(
              "id, status, jobs_found, jobs_matched, applications_sent, started_at, completed_at, errors, user_id",
            )
            .order("started_at", { ascending: false })
            .limit(200),
        ]);

      const runs: RecentAgentRun[] = (agentRunsRes.data ||
        []) as RecentAgentRun[];
      const today = new Date().toISOString().split("T")[0];
      const profiles = profilesRes.data || [];

      const activeToday = profiles.filter(
        (p) => p.last_active_at && p.last_active_at.startsWith(today),
      ).length;
      ...
      // Avg processing time
      const completed = runs.filter((r) => r.completed_at && r.started_at);
      if (completed.length > 0) {
        const avg =
          completed.reduce((sum: number, r) => {
            return (
              sum +
  ...
  const successRate =
    stats && stats.totalAgentRuns > 0
      ? Math.round(
          ((stats.totalAgentRuns - stats.failedAgentRuns) /
            stats.totalAgentRuns) *
            100,
        )
      : 0;
  ...
  const pieData = [
    {
      name: "Success",
      value: (stats?.totalAgentRuns ?? 0) - (stats?.failedAgentRuns ?? 0),
    },
  ...
  const alerts = [
    {
      key: "failed1h",
      status:
        failedLast1h === 0 ? "green" : failedLast1h < 3 ? "yellow" : "red",
      label: "Failed Runs (1h)",
      detail:
        failedLast1h === 0
          ? "No failures in last hour"
          : `${failedLast1h} failed run(s) in last hour`,
    ...
    <div className="max-w-6xl mx-auto px-6 py-8 space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
        {alerts.map((alert) => (
          <div
            ...
            tabIndex={0}
            onKeyDown={(e) => {
              if (e.key === "Enter") navigate(alert.link);
            }}
          >
            {alert.status === "green" ? (
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
        ...
        <StatCard
          ...
          color={errorRate > 20 ? "text-destructive" : "text-warning"}
          suffix="%"
          onClick={() => navigate("/admin/logs?filter=error")}
        />
        <StatCard
        ...
        <Card className="border-border lg:col-span-2">
          <CardHeader className="pb-2">
            <CardTitle className="text-sm flex items-center gap-2">
              <TrendingUp className="w-4 h-4 text-accent" /> Daily Active Users
              (7 days)
            </CardTitle>
          </CardHeader>
          <CardContent>
      ...
      <Card className="border-border">
        <CardHeader className="pb-2">
          <CardTitle className="text-sm flex items-center gap-2">
            <BarChart3 className="w-4 h-4 text-accent" /> Total Runs Over Time
            (7 days)
          </CardTitle>
        </CardHeader>
        <CardContent>
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <div>
                        <p className="text-xs font-medium text-foreground">
                          {run.jobs_found} found · {run.jobs_matched} matched ·{" "}
                          {run.applications_sent} applied
                        </p>
                        <p className="text-[10px] text-muted-foreground">
