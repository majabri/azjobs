...
const STATUS_CONFIG: Record<string, { icon: React.ReactNode; color: string }> = {
  completed: { icon: <CheckCircle2 className="w-3.5 h-3.5" />, color: "text-success border-success/30" },
  failed: { icon: <XCircle className="w-3.5 h-3.5" />, color: "text-destructive border-destructive/30" },
  running: { icon: <Clock className="w-3.5 h-3.5 animate-spin" />, color: "text-accent border-accent/30" },
  pending: { icon: <Clock className="w-3.5 h-3.5" />, color: "text-muted-foreground border-border" },
  completed_with_errors: { icon: <AlertTriangle className="w-3.5 h-3.5" />, color: "text-warning border-warning/30" },
};
    ...
    <div className="max-w-7xl mx-auto px-6 py-8 space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground text-sm mt-1">Monitor and control agent run executions</p>
        </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
        {(["completed", "running", "failed", "pending"] as const).map((status) => {
          ...
          return (
            <div key={status} className="bg-card rounded-xl p-4 border border-border shadow-sm">
            ...
            <table className="w-full text-xs">
              <thead>
                <tr className="border-b border-border bg-muted/30">
                  ...
                  <th className="text-left px-4 py-2.5 font-medium text-muted-foreground">Jobs</th>
                ...
                {runs.map((run) => {
                  ...
                  return (
                    <tr key={run.id} className="border-b border-border last:border-0 hover:bg-muted/20">
                      ...
                      <td className="px-4 py-2.5 text-muted-foreground">
                        {run.jobs_found}f / {run.jobs_matched}m / {run.applications_sent}a
                      </td>
                      <td className="px-4 py-2.5 text-muted-foreground">
          ...
          <DialogContent className="max-w-2xl">
            <DialogHeader>
              <DialogTitle className="flex items-center gap-2 text-sm">
                <Bot className="w-4 h-4" /> Run Details — {selectedRun.id.slice(0, 12)}…
              </DialogTitle>
            </DialogHeader>
            <div className="space-y-4 text-xs">
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <Badge variant="outline" className={`mt-1 gap-1 ${(STATUS_CONFIG[selectedRun.status] || STATUS_CONFIG.pending).color}`}>
                    {selectedRun.status}
                  </Badge>
                </div>
                <div>
                  <Label className="text-muted-foreground text-[10px] uppercase">Duration</Label>
                  <p className="mt-1 font-mono">{duration(selectedRun.started_at, selectedRun.completed_at)}</p>
                </div>
                <div>
                  <Label className="text-muted-foreground text-[10px] uppercase">Started</Label>
                  <p className="mt-1 font-mono">{new Date(selectedRun.started_at).toLocaleString()}</p>
                </div>
                <div>
                  <Label className="text-muted-foreground text-[10px] uppercase">User ID</Label>
                  <p className="mt-1 font-mono">{selectedRun.user_id}</p>
                </div>
              </div>
              <div className="grid grid-cols-3 gap-3">
                <div className="bg-muted/30 rounded p-2 text-center">
                  <div className="text-lg font-bold">{selectedRun.jobs_found}</div>
                  <div className="text-[10px] text-muted-foreground">Jobs Found</div>
                </div>
                <div className="bg-muted/30 rounded p-2 text-center">
                  <div className="text-lg font-bold">{selectedRun.jobs_matched}</div>
                  <div className="text-[10px] text-muted-foreground">Matched</div>
                </div>
                <div className="bg-muted/30 rounded p-2 text-center">
                  <div className="text-lg font-bold">{selectedRun.applications_sent}</div>
                  <div className="text-[10px] text-muted-foreground">Applied</div>
                </div>
              </div>
              {selectedRun.errors?.length > 0 && (
                <div>
                  <Label className="text-muted-foreground text-[10px] uppercase">Errors</Label>
                  <div className="mt-1 space-y-1 font-mono">
                    {selectedRun.errors.map((e, i) => (
                      <p key={i} className="text-destructive bg-destructive/10 px-2 py-1 rounded text-[10px]">{e}</p>
                    ))}
                  </div>
                </div>
              )}
              <div className="flex gap-2">
                {(selectedRun.status === "failed" || selectedRun.status === "completed_with_errors") && (
                  <Button size="sm" onClick={() => { retryRun(selectedRun.id); setSelectedRun(null); }}>
                    <RefreshCw className="w-3.5 h-3.5 mr-1.5" /> Retry Run
                  </Button>
                )}
                {selectedRun.status === "running" && (
                  <Button size="sm" variant="destructive" onClick={() => { cancelRun(selectedRun.id); setSelectedRun(null); }}>
      ...
      <Dialog open={showManual} onOpenChange={setShowManual}>
        <DialogContent className="max-w-xl">
          ...
          <div className="space-y-4">
            ...
            {manualResult && (
              <div>
                <Label className="text-[10px] uppercase text-muted-foreground">Output</Label>
