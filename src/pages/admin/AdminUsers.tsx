...
> = {
  job_seekers: {
    ...
    icon: Target,
    description:
      "Manage job seeker profiles, automation settings, and activity",
  },
  hiring_managers: {
    label: "Hiring Managers",
    icon: Briefcase,
    description:
      "Manage recruiter accounts, job postings, and interview pipelines",
  },
  admins: {
    label: "Admins",
    icon: Shield,
...
export default function AdminUsers() {
  ...
  const handleBulkDelete = async () => {
    ...
    const results = await Promise.allSettled(
      panelRecords.map((user) =>
        callAdminManageUser({ action: "delete", userId: user.user_id }),
      ),
    );
  ...
  return (
    <div className="max-w-6xl mx-auto px-6 py-8 space-y-6">
        ...
        <div className="flex items-center gap-2">
          ...
          <DropdownMenu>
            ...
            <DropdownMenuContent align="end" className="w-52">
              {(Object.entries(VIEW_META) as [AdminView, typeof meta][]).map(
                ([key, m]) => (
                  <DropdownMenuItem
                    key={key}
                    onClick={() => {
      ...
      <AlertDialog
        open={showBulkDelete}
        onOpenChange={(v) => {
          if (!v && !bulkDeleting) setShowBulkDelete(false);
        }}
      >
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>
              Delete all {meta.label.toLowerCase()}?
            </AlertDialogTitle>
            <AlertDialogDescription>
              This will permanently delete all{" "}
              <strong>{panelRecords.length}</strong> {meta.label.toLowerCase()}{" "}
              and all their data. This action <strong>cannot</strong> be undone.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            ...
            >
              {bulkDeleting
                ? "Deleting…"
                : `Delete All ${panelRecords.length} Users`}
            </AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
      </AlertDialog>
