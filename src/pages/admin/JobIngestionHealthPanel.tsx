...
function freshnessVariant(
  ...
  const hoursAgo =
    (Date.now() - new Date(latestScrape).getTime()) / (1000 * 60 * 60);
      ...
      if (!sourceData) {
        ...
        if (raw) {
          const bySource = new Map<
            string,
            { count: number; latest: string; oldest: string }
          >();
          for (const row of raw) {
            ...
            if (!existing) {
              ...
              existing.count++;
              if (scrapedAt > existing.latest) existing.latest = scrapedAt;
              if (scrapedAt < existing.oldest) existing.oldest = scrapedAt;
            }
          }
          const rows: SourceRow[] = Array.from(bySource.entries())
            .map(([source, v]) => ({
              source,
              row_count: v.count,
              latest_scrape: v.latest,
              oldest_scrape: v.oldest,
            }))
        ...
        setTotalJobs(
          sourceData.reduce((s: number, r: SourceRow) => s + r.row_count, 0),
        );
      }
      ...
      <CardContent className="space-y-5">
              ...
              <table className="w-full text-xs">
                ...
                <tbody className="divide-y divide-border">
                  {sources.map((row) => (
                    ...
                    >
                      <td className="px-3 py-2 font-mono text-foreground capitalize">
                        {row.source}
                      </td>
                      ...
                      <td className="px-3 py-2 text-right text-muted-foreground">
                        {formatDistanceToNow(new Date(row.latest_scrape), {
                          addSuffix: true,
                        })}
                      </td>
                      <td className="px-3 py-2 text-center">
                        <Badge
                          variant={freshnessVariant(row.latest_scrape)}
                          className="text-xs"
                        >
        ...
        <div>
          <div className="flex items-center gap-2 mb-2">
            ...
            <p className="text-sm font-medium text-foreground">
              Discovery Agent → discovery_jobs
            </p>
          </div>
          {loading && discovery.length === 0 ? (
            ...
            <p className="text-xs text-muted-foreground py-3 text-center">
              No staging jobs yet — trigger discovery-agent to populate
            </p>
          ) : (
            <div className="flex flex-wrap gap-2">
              {discovery.map((row) => (
                <div
                  key={row.source_board}
                  className="flex items-center gap-1.5 border border-border rounded-md px-2.5 py-1.5"
                >
                  <span className="text-xs font-mono capitalize text-foreground">
                    {row.source_board}
                  </span>
                  <Badge
                    variant="secondary"
