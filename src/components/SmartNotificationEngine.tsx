...
export default function SmartNotificationEngine() {
    ...
    for (const offer of (offers || [])) {
      ...
      if (daysLeft > 0 && daysLeft <= 3) {
        notifications.push({
          user_id: userId,
          type: "urgent",
          title: `⏰ Offer expires in ${daysLeft} day${daysLeft > 1 ? "s" : ""}`,
          message: `Your offer from ${offer.company} for ${offer.job_title} needs a decision soon.`,
          action_url: "/offers",
        });
      }
    }

    // 2. Under-market salary nudge
    const { data: profile } = await supabase
      .from("job_seeker_profiles")
      .select("salary_target, salary_min")
      .eq("user_id", userId)
      .maybeSingle();
      ...
      if (targetNum > 0) {
        ...
        if (avgScore > 0 && avgScore < 60) {
          notifications.push({
            user_id: userId,
            type: "insight",
            title: "💰 You may be under-targeting",
            message: `Your avg fit score is ${avgScore}%. Optimizing your resume could unlock roles paying $${Math.round(targetNum * 1.15).toLocaleString()}+.`,
            action_url: "/job-seeker",
          });
        }
      }
    }

    // 3. Missed opportunity alerts (stale applications)
  ...
  useEffect(() => {
    const timer = setTimeout(generate, 2000); // Delay to avoid blocking initial render
