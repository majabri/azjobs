...
export default function ResumeVault() {
  ...
  const [newVersion, setNewVersion] = useState<ResumeVersion>({
    version_name: "",
    job_type: "",
    resume_text: "",
  });
  ...
  const displayVersions =
    editingVersion !== null ? versions : versionsData || [];
  ...
  return (
    <section>
      ...
      <p className="text-sm text-muted-foreground mb-4">
        Create different versions tailored for different job types. Your latest
        version auto-loads in the Analyze tool.
      </p>
      <div className="space-y-3">
        ...
        {!isLoading && displayVersions.length === 0 && !showNewVersion && (
          <p className="text-sm text-muted-foreground py-2">
            No resume versions yet. Click "Add Resume Version" below to get
            started.
          </p>
        )}
            ...
            {editingVersion === i ? (
              <div className="space-y-3">
                <div className="grid sm:grid-cols-2 gap-3">
                  <div>
                    ...
                    <Input
                      ...
                      onChange={(e) => {
                        const u = [...versions];
                        u[i] = { ...u[i], version_name: e.target.value };
                        setVersions(u);
                      }}
                    />
                  </div>
                  <div>
                    ...
                    <Input
                      ...
                      onChange={(e) => {
                        const u = [...versions];
                        u[i] = { ...u[i], job_type: e.target.value };
                        setVersions(u);
                      }}
                      placeholder="e.g. remote, technical"
                    />
                  </div>
                </div>
                <div>
                  ...
                  <Textarea
                    ...
                    onChange={(e) => {
                      const u = [...versions];
                      u[i] = { ...u[i], resume_text: e.target.value };
                      setVersions(u);
                    }}
                    rows={6}
                  />
                ...
                <div className="flex gap-2">
                  <Button size="sm" onClick={() => saveVersion(v)}>
              ...
              <div className="flex items-start justify-between">
                ...
                <div className="flex gap-1">
                  ...
                  <Button
                    ...
                    onClick={() => v.id && deleteVersion(v.id)}
                  >
        ...
        <Card className="p-4 mt-3 space-y-3">
          <div className="grid sm:grid-cols-2 gap-3">
            ...
            <div>
              <Label>Job Type</Label>
              <Input
                value={newVersion.job_type}
                onChange={(e) =>
                  setNewVersion({ ...newVersion, job_type: e.target.value })
                }
          ...
          <div>
            <Label>Resume Text</Label>
            <Textarea
              value={newVersion.resume_text}
              onChange={(e) =>
                setNewVersion({ ...newVersion, resume_text: e.target.value })
              }
              rows={6}
              placeholder="Paste or write resume here..."
            />
          </div>
          <div className="flex gap-2">
            <Button
              size="sm"
              onClick={() => saveVersion(newVersion)}
              disabled={!newVersion.version_name.trim()}
            >
              <Save className="w-3.5 h-3.5 mr-1" /> Save
            </Button>
            <Button
