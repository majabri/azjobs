...
export default function JobBoardImporter() {
  ...
  const parseJobsFromText = (text: string): ImportedJob[] => {
    ...
    return sections
      .map((section) => {
        ...
        let title =
          lines[0]?.replace(/^\d+[.)]\s*/, "").trim() || "Unknown Role";
        let company = "";
        ...
        if (atMatch) {
          ...
          title = dashMatch[1];
          company = dashMatch[2];
        } else if (lines[1] && lines[1].length < 60) {
  ...
  const handleImport = () => {
    if (!rawInput.trim()) {
      toast.error("Paste your saved jobs or application history");
      return;
    }
    const jobs = parseJobsFromText(rawInput);
    if (!jobs.length) {
      toast.error("Couldn't parse any jobs. Try pasting with more detail.");
      return;
    }
  ...
  const analyzeAll = async () => {
    ...
    try {
      ...
      if (!resumeText) {
        toast.error("No resume found. Upload one first.");
        return;
      }
      ...
      const missedPct =
        importedJobs.length > 0
          ? Math.round((missedCount / importedJobs.length) * 100)
          : 0;
      toast.success(
        `Analysis complete! You missed ${missedPct}% of opportunities.`,
      );
    } catch (e) {
  ...
  const missedOpportunities = doneJobs.filter(
    (j) => (j.originalScore || 0) >= 60,
  );
  const missedPct =
    doneJobs.length > 0
      ? Math.round((missedOpportunities.length / doneJobs.length) * 100)
      : 0;
      ...
      <div className="flex items-center gap-2 mb-4">
        ...
        <div>
          <h3 className="font-display font-bold text-foreground">
            Import My Job Search
          </h3>
          <p className="text-xs text-muted-foreground">
            Re-analyze saved jobs from Indeed, LinkedIn, or other boards
          </p>
        </div>
      </div>
          ...
          {doneJobs.length > 0 && (
            <div className="bg-destructive/5 border border-destructive/20 rounded-xl p-4 text-center">
              ...
              <p className="text-sm text-muted-foreground">
                {missedOpportunities.length} of {doneJobs.length} jobs were
                strong matches that could have been optimized.
              </p>
              <p className="text-xs text-accent mt-2">
                AI can rewrite your resume for each — fixing what you missed.
              </p>
            </div>
          )}
              ...
              >
                {analyzing ? (
                  <>
                    ...
                    {importedJobs.length} jobs...
                  </>
                ) : (
                  <>
                    <Target className="w-4 h-4 mr-2" /> Re-Analyze All Jobs
                  </>
                )}
              </Button>
            )}
            ...
            {importedJobs.map((job) => (
              <div
                ...
                className={`p-3 rounded-lg border ${
                  job.status === "done" && (job.originalScore || 0) >= 60
                    ...
                    <div className="flex items-center gap-2">
                      <div
                        className={`text-lg font-display font-bold ${
                          job.originalScore >= 70
                            ? "text-success"
                            : job.originalScore >= 50
                              ? "text-warning"
                              : "text-destructive"
                        }`}
                      >
                ...
                {job.status === "done" &&
                  ...
                  job.originalScore >= 60 && (
                    <Button
                      ...
                      onClick={() => {
                        navigate("/job-seeker", {
                          state: { prefillJob: job.description },
                        });
                      }}
                    >
                      <ExternalLink className="w-3 h-3 mr-1" /> Optimize &
                      Re-Apply
                    </Button>
                  )}
              </div>
