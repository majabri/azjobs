...
export default function EnhancedServiceHealthPanel() {
    ...
    try {
      ...
      setResponseHistory((prev) => {
        ...
        const avgMs =
          rows.length > 0
            ? Math.round(
                rows.reduce((s, r) => s + (r.error_count > 0 ? 500 : 100), 0) /
                  rows.length,
              )
            : 0;
  ...
  const dbService = services.find(
    (s) =>
      ...
      s.service_name.toLowerCase().includes("db"),
  );
  ...
  const avgResponseTime =
    services.length > 0
      ? Math.round(
          services.reduce((s, r) => s + (r.response_time_ms || 0), 0) /
            services.length,
        )
      : 0;
        ...
        <Card className="border-border">
          <CardContent className="p-4 flex items-center gap-3">
            {STATUS_CONFIG[dbStatus] ? (
              <>
                {(() => {
                  const I = STATUS_CONFIG[dbStatus].icon;
                  return (
                    <I className={`w-6 h-6 ${STATUS_CONFIG[dbStatus].color}`} />
                  );
                })()}
                <div>
                  ...
                  <p
                    className={`font-semibold ${STATUS_CONFIG[dbStatus].color}`}
                  >
                    {STATUS_CONFIG[dbStatus].label}
                  </p>
                </div>
              </>
            ) : (
          ...
          {loading ? (
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
              {edgeFnStatuses.map((fn) => {
                const config =
                  STATUS_CONFIG[fn.status] || STATUS_CONFIG.healthy;
                const Icon = config.icon;
                return (
                  <div
                    key={fn.name}
                    className="flex items-center gap-2.5 p-2.5 bg-muted/20 rounded-lg border border-border"
                  >
