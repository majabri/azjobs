    ...
    try {
      ...
      const [appsRes, historyRes, offersRes] = await Promise.all([
        supabase
          ...
          .select("status, interview_stage, applied_at")
        ...
        supabase
          .from("offers")
          .select("base_salary, bonus, equity, total_comp, market_rate, status")
          .eq("user_id", session.user.id),
      ]);
      ...
      const offers = (offersRes.data || []) as {
        base_salary: number;
        bonus: number;
        equity: number;
        total_comp: number;
      ...
      if (acceptedOffers.length > 0) {
        let bestComp = 0;
        let bestOfferData = acceptedOffers[0];
        for (const o of acceptedOffers) {
          const comp =
            o.total_comp ||
            (o.base_salary || 0) + (o.bonus || 0) + (o.equity || 0);
          if (comp > bestComp) {
            bestComp = comp;
            bestOfferData = o;
          }
        ...
        if (bestOfferData.market_rate > 0) {
          bestOfferVsMarket = Math.round(
            ((bestComp - bestOfferData.market_rate) /
              bestOfferData.market_rate) *
              100,
          );
          marketValueDelta = bestComp - bestOfferData.market_rate;
        }
      } else if (offers.length > 0) {
        let bestComp = 0;
        let bestOfferData = offers[0];
      ...
      const scoreComponent = Math.min(
        100,
        Math.max(0, 50 + fitScoreGrowth * 2),
      );
      const interviewComponent = Math.min(
        100,
        interviewConversionImprovement * 3,
      );
      const compComponent =
      ...
      if (recentScores.length > 0 && olderScores.length > 0) {
        const recentAvg =
          ...
          recentScores.length;
        const olderAvg =
          olderScores.reduce((s, h) => s + h.overall_score, 0) /
          olderScores.length;
        weeklyTrend =
