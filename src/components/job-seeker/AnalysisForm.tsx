      ...
      <div className="flex-1 min-w-0">
        ...
        {alert.showBrowserExtract && onOpenTab && onPasteClipboard && (
          <div className="mt-2 flex gap-2 flex-wrap">
            <button onClick={onOpenTab} className="rounded-md bg-current/10 px-2.5 py-1 text-xs font-medium hover:bg-current/20 transition-colors">
              Open job tab ↗
            </button>
            <button onClick={onPasteClipboard} className="rounded-md bg-current/10 px-2.5 py-1 text-xs font-medium hover:bg-current/20 transition-colors">
...
export default function AnalysisForm({ onAnalyze, isAnalyzing, isDemo, prefillJob, prefillJobLink }: AnalysisFormProps) {
  ...
  const handleScrapeResult = useCallback(
    (result: Awaited<ReturnType<typeof scrapeUrl>>) => {
      ...
      const needsBrowser = err.toLowerCase().includes("browser") || err.toLowerCase().includes("workday") || err.toLowerCase().includes("blocks server") || err.toLowerCase().includes("requires a browser") || err.toLowerCase().includes("login") || err.toLowerCase().includes("ip-block");
  ...
  const handleOpenTab = () => {
    if (jobLink.trim()) {
      ...
      setUrlAlert({
        variant: 'warning',
        message: 'Job opened in new tab — press Ctrl+A then Ctrl+C on that tab to copy all text, then click "Paste from clipboard" below.',
  ...
  const handlePasteClipboard = async () => {
    try {
      ...
      if (text && text.trim().length > 100) {
        ...
        setUrlAlert({ variant: 'error', message: 'Clipboard appears empty. Copy the job page text first (Ctrl+A, Ctrl+C), then try again.' });
      }
    } catch {
      setUrlAlert({ variant: 'error', message: 'Clipboard access was denied. Please paste the job description directly into the text box below.', showBrowserExtract: false });
      setTimeout(() => jobDescRef.current?.focus(), 50);
    }
  };
      ...
      {isAnalyzing && (
        <div className="mt-6 max-w-sm mx-auto">
          <div className="h-1.5 bg-secondary rounded-full overflow-hidden">
            <div className="h-full gradient-indigo rounded-full animate-pulse" style={{ width: "60%" }} />
          </div>
