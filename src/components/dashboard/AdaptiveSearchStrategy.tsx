...
export default function AdaptiveSearchStrategy() {
  ...
  const loadStrategy = async () => {
    ...
    try {
      ...
      const ghosted = allApps.filter(
        (a) => a.status === "applied" && a.response_days === null,
      ).length;
      ...
      const recentAvg = recentScores.length
        ? recentScores.reduce((s: number, v: number) => s + v, 0) /
          recentScores.length
        : 0;
      const olderAvg = olderScores.length
        ? olderScores.reduce((s: number, v: number) => s + v, 0) /
          olderScores.length
        : 0;
      const weeklyTrend =
        recentAvg > olderAvg + 3
          ? "improving"
          : recentAvg < olderAvg - 3
            ? "declining"
            : "stable";
      ...
      if (totalApps > 5 && conversionRate < 15) {
        insights.push({
          ...
          detail: `Only ${conversionRate}% of your applications get interviews. Focus on fewer, higher-match jobs rather than volume.`,
        });
      }
      if (totalApps > 5 && conversionRate >= 25) {
        insights.push({
          ...
          detail: `${conversionRate}% interview rate is above average. Your targeting is working well.`,
        });
      }
      if (ghosted > totalApps * 0.5 && totalApps > 3) {
        insights.push({
          ...
          detail: `${Math.round((ghosted / totalApps) * 100)}% of applications got no response. Try following up within 5 days and applying to smaller companies.`,
        });
      }
      if (rejections > interviews && totalApps > 3) {
        insights.push({
          ...
          detail:
            "More rejections than interviews suggests your resume isn't matching ATS keywords. Use the AI optimizer for each application.",
        });
      }
      ...
      allAnalyses.forEach((a) => {
        (Array.isArray(a.gaps) ? a.gaps : []).forEach((g: any) => {
          if (g && typeof g === "object" && "area" in g) {
            gapCounts[g.area as string] =
              (gapCounts[g.area as string] || 0) + 1;
          }
        });
      });
      ...
      if (topGaps.length > 0) {
        insights.push({
          ...
          detail: `You're consistently missing: ${topGaps.map(([k]) => k).join(", ")}. Adding these skills or keywords to your resume could significantly improve match rates.`,
        });
      }
      ...
      <Card className="p-6">
        <div className="flex items-center justify-center py-8">
          ...
          <span className="text-sm text-muted-foreground">
            Analyzing your search strategy...
          </span>
        </div>
      </Card>
    );
  ...
  const trendIcon =
    strategy.weeklyTrend === "improving"
      ? "📈"
      : strategy.weeklyTrend === "declining"
        ? "📉"
        : "📊";
  const trendColor =
    strategy.weeklyTrend === "improving"
  ...
  return (
    <Card className="p-6">
            ...
            <h3 className="font-display font-bold text-foreground flex items-center gap-1.5">
              ...
              {strategy.weeklyTrend === "improving"
                ...
                : strategy.weeklyTrend === "declining"
                  ? "Needs Work"
                  : "Stable"}{" "}
              <HelpTooltip text="Tracks your job search performance week over week — conversion rates, interview success, and keyword effectiveness — and suggests adjustments." />
            </h3>
            <p className="text-xs text-muted-foreground">
              Weekly Search Strategy Update
            </p>
          </div>
        </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-5 gap-3 mb-5">
        ...
        <div className="bg-muted/30 rounded-lg p-3 text-center border border-border">
          <div
            className={`text-xl font-display font-bold ${strategy.conversionRate >= 20 ? "text-success" : strategy.conversionRate >= 10 ? "text-warning" : "text-destructive"}`}
          >
      ...
      <div className={`flex items-center gap-2 mb-4 text-sm ${trendColor}`}>
        ...
        <span className="text-xs text-muted-foreground">
          • Avg fit score: {strategy.avgScore}%
        </span>
      </div>
