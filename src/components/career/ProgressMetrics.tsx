...
function computeMetrics(
  apps: { applied_at: string | null; status: string | null; interview_stage?: string | null }[],
  ...
  if (sortedHistory.length > 0) {
    avgScore = Math.round(sortedHistory.reduce((s, h) => s + h.overall_score, 0) / sortedHistory.length);
    if (sortedHistory.length >= 4) {
      const half = Math.floor(sortedHistory.length / 2);
      const oldAvg = sortedHistory.slice(0, half).reduce((s, h) => s + h.overall_score, 0) / half;
      const newAvg = sortedHistory.slice(half).reduce((s, h) => s + h.overall_score, 0) / (sortedHistory.length - half);
...
export default function ProgressMetrics() {
  ...
  const { data: historyData = [], isLoading: historyLoading } = useAnalysisHistory(50);
  ...
  const metrics = computeMetrics(
    appsData.map(a => ({
      ...
      interview_stage: (a as Record<string, string | null>)["interview_stage"] ?? null,
    })),
    historyData.map(h => ({ overall_score: h.overall_score, created_at: h.created_at })),
  );
