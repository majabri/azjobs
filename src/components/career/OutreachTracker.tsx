...
const STATUS_OPTIONS = [
  ...
  { value: "replied", label: "Replied", icon: CheckCircle2, color: "text-success" },
  { value: "no_response", label: "No Response", icon: X, color: "text-destructive" },
];
  ...
  return (
    <Card className="p-6">
      <div className="flex items-center justify-between mb-4">
        <h3 className="font-display font-bold text-primary">Outreach Tracker</h3>
      ...
      <div className="flex gap-1.5 mb-4">
        <Badge variant={filter === "all" ? "default" : "outline"} className="cursor-pointer text-xs" onClick={() => setFilter("all")}>All ({contacts.length})</Badge>
        {STATUS_OPTIONS.map(s => {
          const count = contacts.filter(c => c.response_status === s.value).length;
          return (
            <Badge key={s.value} variant={filter === s.value ? "default" : "outline"} className="cursor-pointer text-xs" onClick={() => setFilter(s.value)}>
              {s.label} ({count})
            </Badge>
          );
        })}
      ...
      {loading ? (
        ...
        <p className="text-sm text-muted-foreground text-center py-6">No outreach contacts yet. Start networking!</p>
      ) : (
        <div className="space-y-2">
          {filtered.map(c => (
            <div key={c.id} className="flex items-center justify-between p-3 rounded-lg bg-card border border-border hover:border-accent/20 transition-colors">
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2">
                  <span className="font-medium text-foreground text-sm truncate">{c.contact_name}</span>
                  <span className="text-xs text-muted-foreground">at {c.company}</span>
                ...
                <div className="flex items-center gap-2 mt-1">
                  ...
                  {c.sent_at && <span className="text-[10px] text-muted-foreground">Sent {new Date(c.sent_at).toLocaleDateString()}</span>}
                </div>
              </div>
              <div className="flex items-center gap-1">
                {STATUS_OPTIONS.map(s => (
                  <Button key={s.value} variant="ghost" size="sm" className={`h-7 px-2 ${c.response_status === s.value ? "bg-accent/10" : ""}`} onClick={() => updateStatus(c.id, s.value)}>
                    <s.icon className={`w-3 h-3 ${s.color}`} />
                  </Button>
                ))}
                <Button variant="ghost" size="sm" className="h-7 px-2 text-muted-foreground hover:text-destructive" onClick={() => deleteContact(c.id)}>
