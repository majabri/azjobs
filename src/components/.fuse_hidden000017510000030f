...
export default function AgentHealthDashboard() {
  ...
  const load = async () => {
    try {
      ...
      const agents = ["discovery", "matching", "optimization", "application", "learning"];
  ...
  return (
    <Card className="p-5">
      <div className="flex items-center gap-2 mb-4">
        ...
        <h3 className="font-display font-bold text-primary text-sm">Agent Health (Last 30 Runs)</h3>
      </div>
      <div className="grid grid-cols-5 gap-2">
        {health.map(h => {
          ...
          return (
            <div key={h.agent} className={`text-center p-3 rounded-lg border ${consecutive ? "border-destructive/30 bg-destructive/5" : "border-border"}`}>
              <p className="text-xs font-medium text-foreground capitalize mb-1">{h.agent}</p>
              <p className={`font-display font-bold text-lg ${color}`}>{rate}%</p>
              <p className="text-[10px] text-muted-foreground">{h.runs} runs</p>
              {h.avgTime > 0 && (
                <p className="text-[9px] text-muted-foreground mt-0.5">
                  <Clock className="w-2.5 h-2.5 inline mr-0.5" />
                  {h.avgTime < 1000 ? `${h.avgTime}ms` : `${(h.avgTime / 1000).toFixed(1)}s`}
                </p>
              )}
              {consecutive && (
                <Badge variant="outline" className="text-[8px] text-destructive border-destructive/30 mt-1">Circuit Break</Badge>
              )}
            </div>
          );
        })}
