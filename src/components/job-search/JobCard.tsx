...
function SalaryBadge({ salary, title }: { salary: string; title?: string }) {
  ...
  const diff =
    ((parsed - estimateMarketRate(title || "")) /
      estimateMarketRate(title || "")) *
    100;
  if (diff >= 10)
    return (
      <Badge
        variant="outline"
        className="text-[10px] bg-green-500/10 text-green-600 border-green-300 dark:text-green-400"
      >
        ↑ Above Market
      </Badge>
    );
...
const AI_SMART_TAG_CONFIG: Record<
  string,
...
> = {
  hot_match: {
    label: "Hot Match",
    color:
      "text-green-600 border-green-300 dark:text-green-400 bg-green-500/10",
    icon: TrendingUp,
  },
  good_fit: {
    label: "Good Fit",
  ...
  stretch: {
    label: "Stretch",
    color: "text-amber-600 border-amber-300 dark:text-amber-400",
    icon: Zap,
  },
  reach: {
    label: "Reach",
    color: "text-orange-600 border-orange-300 dark:text-orange-400",
    icon: Shield,
  },
  low_roi: {
    label: "Low ROI",
    color: "text-destructive/70 border-destructive/20",
    icon: AlertTriangle,
  },
  apply_fast: {
    label: "Apply Fast",
...
function getSmartTagUI(job: EnrichedJob): {
  label: string;
  color: string;
  ...
  if ((job.effortEstimate || 0) > 70 && prob < 40)
    return {
      label: "Low ROI",
      color: "text-destructive/70 border-destructive/20",
      icon: AlertTriangle,
  ...
  if (prob >= 50 && job.first_seen_at) {
    const days =
      (Date.now() - new Date(job.first_seen_at).getTime()) /
      (1000 * 60 * 60 * 24);
    if (days < 3)
      return {
        label: "Apply Fast",
...
export function JobCard({
  ...
  const displayProb = hasAiScore
    ? (job.response_prob ?? job.responseProbability ?? 0)
    : (job.responseProbability ?? 0);
  const tag = getSmartTagUI(job);
  const TagIcon = tag.icon;
  const trustCfg = job.trustLevel
    ? TRUST_LEVEL_CONFIG[job.trustLevel]
    : TRUST_LEVEL_CONFIG.trusted;
  const TrustIcon =
    trustCfg.icon === "shield-check"
      ? ShieldCheck
      : trustCfg.icon === "shield-alert"
        ? ShieldAlert
        : ShieldX;
  const hasFlags = job.flags && job.flags.length > 0;
  const hasDanger = job.flags?.some((f) => f.severity === "danger");
  const hasSkillData =
    (job.matched_skills?.length ?? 0) > 0 || (job.skill_gaps?.length ?? 0) > 0;
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex flex-wrap items-center gap-3 mt-3">
            {hasAiScore && (
              <div
                className={
                  ...
                  getFitScoreBg(job.fit_score!)
                }
              >
                <Sparkles
                  className={"w-3.5 h-3.5 " + getFitScoreColor(job.fit_score!)}
                />
                <span
                  className={
                    "text-sm font-bold " + getFitScoreColor(job.fit_score!)
                  }
                >
                  {job.fit_score}% fit
                </span>
            ...
            <div className="flex items-center gap-1">
              ...
              <span className={"text-xs font-semibold " + trustCfg.colorClass}>
                {trustCfg.label}
              </span>
            </div>
            ...
            {displayProb > 0 && (
              <span
                className={"text-sm font-semibold " + getProbColor(displayProb)}
              >
                {displayProb}% response prob.
              </span>
            )}
          </div>
                  ...
                  {(job.matched_skills?.length ?? 0) > 0 && (
                    <div>
                      <p className="text-xs font-semibold text-green-600 dark:text-green-400 mb-1 flex items-center gap-1">
                        <CheckCircle className="w-3 h-3" /> Skills you have (
                      ...
                      <div className="flex flex-wrap gap-1">
                        {job.matched_skills!.map((s, i) => (
                          <Badge
                            key={i}
                            variant="outline"
                            className="text-[10px] bg-green-500/10 text-green-600 border-green-300/50 dark:text-green-400"
                          >
                            {s}
                          </Badge>
                        ))}
                  ...
                  {(job.skill_gaps?.length ?? 0) > 0 && (
                    <div>
                      <p className="text-xs font-semibold text-amber-600 dark:text-amber-400 mb-1 flex items-center gap-1">
                        <BookOpen className="w-3 h-3" /> Skills to develop (
                      ...
                      <div className="flex flex-wrap gap-1">
                        {job.skill_gaps!.map((s, i) => (
                          <Badge
                            key={i}
                            variant="outline"
                            className="text-[10px] bg-amber-500/10 text-amber-600 border-amber-300/50 dark:text-amber-400"
                          >
                            {s}
                          </Badge>
                        ))}
