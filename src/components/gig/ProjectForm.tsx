...
const EMPTY: FormData = { title: "", description: "", budgetMin: "", budgetMax: "", timelineDays: "", skills: [], deliverables: "" };
...
export default function ProjectForm({ editing, onSaved, onCancel }: Props) {
  ...
  const handleSubmit = async (asDraft = false) => {
    ...
    const payload = {
      ...
      timeline_days: form.timelineDays ? Number(form.timelineDays) : null,
    ...
    else { toast.success(form.id ? "Project updated!" : "Project posted!"); onSaved(); }
    setSaving(false);
  };
  ...
  return (
    <div className="max-w-2xl space-y-5">
      ...
      <div className="flex items-center gap-3 text-sm">
        <button onClick={() => setStep(1)} className={`px-3 py-1.5 rounded-full transition-colors ${step === 1 ? "bg-primary text-primary-foreground" : "bg-muted text-muted-foreground"}`}>
          1. Project Info
        </button>
        <div className="w-8 h-px bg-border" />
        <button onClick={() => { if (validate1()) setStep(2); }} className={`px-3 py-1.5 rounded-full transition-colors ${step === 2 ? "bg-primary text-primary-foreground" : "bg-muted text-muted-foreground"}`}>
          2. Requirements
        </button>
      </div>
      ...
      {step === 1 && (
        <div className="space-y-4">
          <div className="space-y-1.5">
            ...
            <Input value={form.title} onChange={e => update("title", e.target.value)} placeholder="Build a React Dashboard" />
          </div>
          <div className="space-y-1.5">
            <label className="text-xs font-medium text-muted-foreground">Description *</label>
            <Textarea value={form.description} onChange={e => update("description", e.target.value)} className="h-40 resize-none" placeholder="Describe the project scope, goals, and expectations..." />
          </div>
          <div className="grid grid-cols-3 gap-3">
            <div className="space-y-1.5">
              <label className="text-xs font-medium text-muted-foreground">Budget Min ($)</label>
              <Input type="number" value={form.budgetMin} onChange={e => update("budgetMin", e.target.value)} placeholder="1000" />
            </div>
            <div className="space-y-1.5">
              <label className="text-xs font-medium text-muted-foreground">Budget Max ($)</label>
              <Input type="number" value={form.budgetMax} onChange={e => update("budgetMax", e.target.value)} placeholder="5000" />
            </div>
            <div className="space-y-1.5">
              <label className="text-xs font-medium text-muted-foreground">Timeline (days)</label>
              <Input type="number" value={form.timelineDays} onChange={e => update("timelineDays", e.target.value)} placeholder="30" />
            </div>
          </div>
          <div className="flex justify-end">
            <Button onClick={() => { if (validate1()) setStep(2); }} className="gap-2">Next <ArrowRight className="w-4 h-4" /></Button>
          </div>
        </div>
      )}
      ...
      {step === 2 && (
        <div className="space-y-4">
          <div className="space-y-1.5">
            ...
            <SkillsTagInput value={form.skills} onChange={s => update("skills", s)} />
          </div>
          <div className="space-y-1.5">
            <label className="text-xs font-medium text-muted-foreground">Deliverables (one per line)</label>
            <Textarea value={form.deliverables} onChange={e => update("deliverables", e.target.value)} className="h-28 resize-none" placeholder="Responsive dashboard UI\nAPI integration\nDocumentation" />
          </div>
          <div className="flex items-center justify-between pt-2">
            ...
            <div className="flex gap-2">
              ...
              <Button onClick={() => handleSubmit(false)} disabled={saving}>
                {saving && <Loader2 className="w-4 h-4 animate-spin mr-2" />} {form.id ? "Update" : "Post"} Project
              </Button>
            </div>
          </div>
        </div>
