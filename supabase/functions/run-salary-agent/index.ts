...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!profile) {
      return respond({
        snapshot: null,
        hint: "Complete your profile to get salary benchmarks",
      });
    }
    ...
    const userMid =
      userMin && userMax ? (userMin + userMax) / 2 : userMin || userMax;
    ...
    const snapshot = {
      ...
      raw_data: {
        ...
        titles_sampled: titles.slice(0, 5),
      },
    };
