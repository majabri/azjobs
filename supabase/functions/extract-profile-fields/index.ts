...
Deno.serve(async (req) => {
    if (req.method === "OPTIONS") {
      ...
      if (!ANTHROPIC_API_KEY) {
              return new Response(
                        JSON.stringify({ profile: regexFallback(resumeText) }),
                { headers: { ...corsHeaders, "Content-Type": "application/json" } }
                      );
      }

      // Use Anthropic Messages API with tool_use for structured extraction
      const response = await fetch("https://api.anthropic.com/v1/messages", {
              ...
              body: JSON.stringify({
                        ...
                        max_tokens: 4096,
                        system:
                                    "You are a resume parser. Extract structured profile data from the resume text provided. Be thorough and accurate. For work experience, extract every position listed. For education, extract every degree. For certifications, extract all listed certifications, licenses, and professional credentials.",
                        messages: [
                          {
                                        role: "user",
                        ...
                        tools: [
                          {
                                        ...
                                        description:
                                                        "Extract structured profile data from a resume",
                                        input_schema: {
                                                        type: "object",
                                                        properties: {
                                                                          full_name: {
                                                                          ...
                                                                          summary: {
                                                                                              type: "string",
                                                                                              description:
                                                                                                                    "Professional summary or objective (max 500 chars)",
                                                                          },
                                                                          linkedin_url: {
                                                                                              type: "string",
                                                                          ...
                                                                          education: {
                                                                                              ...
                                                                                              items: {
                                                                                                                    ...
                                                                                                                    properties: {
                                                                                                                                            ...
                                                                                                                                            year: {
                                                                                                                                                                      type: "string",
                                                                                                                                                                      description: "Graduation year or date range",
                                                                                                                                              },
                                                                                                                      },
      ...
      // Parse Anthropic response format (content array with tool_use blocks)
      const toolUseBlock = aiResult.content?.find(
              (block: any) => block.type === "tool_use"
            );

      if (!toolUseBlock?.input) {
              console.error(
                        "No tool_use block in AI response, falling back to regex"
                      );
              return new Response(
                        JSON.stringify({ profile: regexFallback(resumeText) }),
                { headers: { ...corsHeaders, "Content-Type": "application/json" } }
      ...
      // Anthropic already parses tool input as an object
      ...
      const profile = {
              ...
              summary: extracted.summary
                ? extracted.summary.substring(0, 500)
                        : null,
              ...
              skills:
                  ...
                  ? extracted.skills
                          : null,
              work_experience:
                        Array.isArray(extracted.work_experience) &&
...
function regexFallback(resumeText: string) {
    ...
    const fullName =
          lines.length > 0
        ? lines[0].replace(/[^a-zA-Z\s'-]/g, "").trim()
            : null;
  ...
  const locationMatch =
        resumeText.match(/(?:location|address|city)[:\s]*([^\n]+)/i) ||
        resumeText.match(
