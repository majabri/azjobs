...
Deno.serve(async (req) => {
  ...
  try {
    ...
    /* invalid JWT — isAuthorized stays false */
  }
  ...
  for (const p of prefs ?? []) {
    for (const t of p.target_titles ?? []) {
      ...
      if (termSet.size >= 3) break; // cap to 3 terms (5 adapters × 3 terms × 15 results = 225 jobs max)
    }
    if (termSet.size >= 3) break;
  }

  // Fallback terms so the agent is useful before any user preferences exist.
  if (termSet.size === 0) {
    for (const t of ["software engineer", "product manager", "data scientist"])
      termSet.add(t);
  }
  const terms = [...termSet];

  // ── Adapter registry ─────────────────────────────────────────────────────────
  // Order matters for wall-clock budget: run Lever early (it fetches ATS company slugs
  // sequentially and times out if starved); Greenhouse and USAJobs last because they
  // are either reliable or slow API calls.
  const adapters = [
    ...
    try {
      for (const term of terms) {
        const result = await adapter.search({
          search_term: term,
          results_wanted: 15, // 30→15 to stay within WORKER_RESOURCE_LIMIT with 5 adapters active
          is_remote: true,
        });
