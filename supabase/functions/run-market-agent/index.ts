...
const SKILL_PATTERNS = [
  ...
  "Kubernetes","Docker","Terraform","CI/CD","REST API","GraphQL","Machine Learning",
  "Data Science","Cybersecurity","Zero Trust","SIEM","SOAR","Cloud Security",
  "Product Management","Agile","Scrum","Salesforce","HubSpot","Excel","Tableau",
  "Leadership","Communication","Strategic Planning","P&L","Revenue","Go-to-Market",
  "Compliance","GDPR","SOC 2","ISO 27001","Risk Management","Audit",
  "Java","Go","Rust","C++","Swift","Kotlin",".NET","PHP","Ruby",
];

function countSkills(descriptions: string[]): { skill: string; count: number }[] {
  const counts: Record<string, number> = {};
  const combined = descriptions.join(" ").toLowerCase();
  for (const skill of SKILL_PATTERNS) {
    const re = new RegExp(`\\b${skill.toLowerCase().replace(".", "\\.")}\\b`, "g");
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    await pgrest("user_agent_instances", "on_conflict=user_id%2Cagent_type", {
      ...
      body: JSON.stringify({
        user_id: userId, agent_type: "market_intel",
    ...
    // Trending skills
    const descriptions = jobs.map((j: any) => j.description ?? "").filter(Boolean);
