...
const InputSchema = z.object({
  ...
  messageType: z
    .enum(["cold_outreach", "warm_intro", "informational"])
...
serve(async (req) => {
  ...
  try {
    ...
    const typeDescriptions: Record<string, string> = {
      cold_outreach:
        "a cold LinkedIn/email outreach message to a recruiter or hiring manager",
      warm_intro:
        "a warm introduction request asking a mutual connection to introduce them",
      informational:
        "a request for an informational interview to learn about the company/role",
    };
...
- Return ONLY the message text, no subject line or formatting`;
    ...
    const result = await callAnthropic({
      system:
        "You write concise, effective networking messages. Return only the message text.",
      userMessage: prompt,
      temperature: 0.7,
    });
