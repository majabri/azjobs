...
export const usaJobsAdapter: BoardAdapter = {
  ...
  async search(params: SearchParams): Promise<AdapterResult> {
    ...
    if (!apiKey || !userAgent) {
      console.warn('[usajobs] USAJOBS_API_KEY or USAJOBS_USER_AGENT not set — skipping');
    ...
    if (params.is_remote) q.set('RemoteIndicator', 'True');
    ...
    const payload = (await res.json()) as { SearchResult: { SearchResultItems: USAJobsResult[] } };
    ...
    for (const item of items) {
      ...
      if (!matchesSearch({ title: d.PositionTitle, description: desc }, params.search_term)) continue;
      ...
      jobs.push({
        ...
        remote_type:      null,
        employment_type:  d.PositionSchedule?.[0]?.Name?.toLowerCase().includes('full') ? 'full_time' : null,
