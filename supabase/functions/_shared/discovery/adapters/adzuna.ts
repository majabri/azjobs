...
export const adzunaAdapter: BoardAdapter = {
  ...
  async search(params: SearchParams): Promise<AdapterResult> {
    ...
    if (!appId || !appKey) {
      console.warn(
        "[adzuna] ADZUNA_APP_ID or ADZUNA_APP_KEY not set — skipping",
      );
    ...
    // Note: Adzuna's keyword search already filters by `what`; no remote-only
    // filter is applied here because Adzuna US free-tier listings are sparse
    // and the post-filter would eliminate valid results.
    ...
    for (const a of payload.results ?? []) {
      ...
      // Skip matchesSearch — the Adzuna API's `what` parameter already filters
      // by keyword; re-filtering here causes false-zero results on sparse data.

      const empType: NormalisedJob["employment_type"] =
        a.contract_type === "full_time"
          ? "full_time"
          : a.contract_type === "part_time"
            ? "part_time"
            : a.contract_type === "contract"
              ? "contract"
              : null;
      ...
      jobs.push({
        ...
        remote_type: a.location.display_name?.toLowerCase().includes("remote")
          ? "remote"
          : null,
