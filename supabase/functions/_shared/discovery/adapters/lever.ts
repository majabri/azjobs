...
export const leverAdapter: BoardAdapter = {
  ...
  async search(params: SearchParams): Promise<AdapterResult> {
    ...
    // Cap to 3 companies per run — Lever's API can be slow (3-8s per company)
    // and the edge function has a wall-clock limit. Each fetch uses a 6s timeout.
    ...
    for (const c of companyList) {
      ...
      const res = await politeFetch(url, {}, 6_000);
      ...
      for (const p of postings) {
        ...
        const descHtml = [p.description, p.additional]
          .filter(Boolean)
          .join("\n");
        const descPlain =
        ...
        if (
          !matchesSearch(
            { title: p.text, description: descPlain },
            params.search_term,
          )
        )
          continue;
