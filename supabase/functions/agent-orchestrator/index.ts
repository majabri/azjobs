...
async function withRetry(
  ...
  return {
    ...
    success: false,
    metrics: {},
    error: `Failed after ${MAX_RETRIES + 1} attempts: ${lastErr?.message}`,
  };
}
...
async function runDiscovery(ctx: AgentContext): Promise<AgentResult> {
  ...
  return {
    name: "discovery",
    success: true,
    metrics: { jobs_found: result.jobs.length },
  };
}
...
async function runMatching(ctx: AgentContext): Promise<AgentResult> {
      ...
      messages: [
        {
          ...
          content:
            ...
            `Jobs:\n${jobs
              ...
              .map(
                (j: any, i: number) =>
                  `[${i}] ${j.title} @ ${j.company}: ${(j.description || "").slice(0, 200)}`,
              )
              .join("\n")}`,
        },
      ],
  ...
  try {
    ...
    if (jsonMatch) {
      ...
      matched = scores.filter(
        (s: any) => s.match_score >= ctx.config.threshold,
      ).length;
    }
  } catch {
    /* parse error — ok */
...
async function runApplication(ctx: AgentContext): Promise<AgentResult> {
  ...
  const count = await pgCount(
    "job_applications",
    `user_id=eq.${ctx.userId}&applied_at=gte.${today}T00:00:00`,
  );
...
async function runLearning(ctx: AgentContext): Promise<AgentResult> {
  const apps = await pgGet(
    "job_applications",
    `select=status&user_id=eq.${ctx.userId}&order=applied_at.desc&limit=50`,
  );
  ...
  await pgPost(
    "learning_events",
    {
      ...
      insights: {
        interview_rate:
          (((outcomes.interview || 0) / apps.length) * 100).toFixed(1) + "%",
        ghost_rate:
          (((outcomes.ghosted || 0) / apps.length) * 100).toFixed(1) + "%",
      },
    },
    "return=minimal",
  ).catch(() => {});
  ...
  for (const phase of phases) {
    const results = await Promise.allSettled(
      phase.map(async (name) => {
        ...
        ingestLog("info", `Agent ${name} started`, {
          user_id: ctx.userId,
          run_id: runId,
          agent_id: name,
          status: "running",
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // Create run record
    const runs = await pgPost("agent_runs", {
      user_id: user.id,
      status: "running",
      agents_completed: [],
    });
    ...
    ingestLog("info", "Agent orchestrator run started", {
      user_id: user.id,
      run_id: run.id,
      status: "running",
    });
    const { completed, errors, metrics, timings } = await executeAgents(
      validAgents,
      ctx,
      run.id,
    );
